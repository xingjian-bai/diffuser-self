...
def main(args):
    ...
    if accepts_prediction_type:
        ...
        if args.model_type == "unet":
            noise_scheduler = DDPMScheduler(  # ? or DDIM Luke: should be the same
                num_train_timesteps=args.ddpm_num_steps,
                beta_schedule=args.ddpm_beta_schedule,
            )
        elif args.model_type == "transformer":
    ...
    for epoch in range(first_epoch, args.num_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(model):
                ...
                if args.prediction_type == "epsilon":
                    # print(f'shapes before loss: {model_output.shape}, {noise.shape}')
        ...
        if accelerator.is_main_process:
            if epoch % args.save_images_epochs == 0 or epoch == args.num_epochs - 1:
                ...
                if args.use_ema:
                    ema_model.copy_to(model_unwrapped.parameters())
                ...
                def save_images(images):
                    ...
                    for i, image in enumerate(images):
                        Image.fromarray(image).save(f"{folder_name}/image_{i}.png")

                save_images(images_processed)

                score = fid.compute_fid(args.dataset_dir, folder_name)
                ...
                #     )
                # TODO log images_processed as images into wandb, with epoch as the index
                if args.use_wandb:
                    wandb.log(
                        {
